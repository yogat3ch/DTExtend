...
#' @param tab_accessible \code{lgl} Whether to enable row selection by pressing the `Enter` key. Requires `extensions = 'KeyTables'` with `options = list(keys = TRUE)`
...
DT_callback <- function(..., search_icon = TRUE, page_input = TRUE, pageLength_input = TRUE, filter_placeholders = TRUE, retain_policy_color = TRUE, retain_selections = TRUE, tab_accessible = FALSE, debugger = FALSE) {
  ...
  if (tab_accessible) {
    lines <- c(lines, 'dt_tab_accessible(table);')
  }
